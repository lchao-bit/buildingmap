...
L.Transformation.prototype = {
	...
	_transform: function (point, scale) {
		...
		//var t_y = scale * (this._c * point.y + this._d);
		//point.y = 0.97355521387151346807949804786205* t_y;
		return point;
	},
...
L.CRS.Simple = L.extend({}, L.CRS, {
	...
	//transformation: new L.Transformation(0.8, 0, -1, 0),
	//transformation: new L.Transformation(1 / 360, 0.5, -1 / 360, 0.5),
	//transformation: new L.Transformation(0.5 / Math.PI, 0.5, -0.5 / Math.PI, 0.5),
	//transformation: new L.Transformation(1, 0.5, -1, 0.5),
	transformation: new L.Transformation(1 / 360, 0.5, -1 / 276, 0.5),
...
L.Map = L.Class.extend({
	...
	options: {
		crs: L.CRS.EPSG4326,//zc,change it to fit geohash projection //Simple crs: L.CRS.EPSG3857,
 ...
 L.GeohashLayer = L.Class.extend({
	...
	_update: function () {
		...
		/*var tileBounds = L.bounds(
		        bounds.min.divideBy(tileSize)._floor(),
		        bounds.max.divideBy(tileSize)._floor());*/
		//zc,size of lonlat bounds
		var minb = L.CRS.Simple.pointToLatLng(bounds.min,zoom),
				maxb = L.CRS.Simple.pointToLatLng(bounds.max,zoom);
		
		var tileBounds = L.bounds(maxb,minb);

		//this._addTilesFromCenterOut(tileBounds);
		this._addTilesFromCenterOut(bounds,minb,maxb);
	...
	_addTilesFromCenterOut: function (bounds,latlng1,latlng2) {
		var queue = [],
		    center = bounds.getCenter();
		...
		var min_x = Math.min(latlng1.lat, latlng2.lat);
		...
		/*for(var x = latlng1.lat; x < max_x + 0.01 && x > min_x - 0.01; x+=0.01 * step_x){
			for(var y = latlng1.lng; y < max_y + 0.01 && y > min_y - 0.001; y+=0.01 * step_y){
				geohash = encode_geohash(y, x);
		...
		//zc,暂时使用固定的4块地图数据
		//var tmpgeohash = ["wx4er1","wx4er4","wx4er5","wx4erj"];
		var tmpgeohash = ["wx4er5"];
		for(var i = 0; i<tmpgeohash.length;i++){
			queue.push(tmpgeohash[i]);
		}
